...
class _CoordiRepository implements CoordiRepository {
  _CoordiRepository(this._dio, {this.baseUrl, this.errorLogger});
  ...
  Future<ApiResponse<CoordiPostingResult>> uploadCoordi({
    required dynamic postingSaveDTO,
  }) async {
  ...
  Future<ApiResponse<CoordiPostingResult>> uploadImage({
    ...
    _data.fields.add(MapEntry('linkDTO', linkDTO));
    _data.files.addAll(
      multipartFiles.map((i) => MapEntry('multipartFiles', i)),
    );
    final _options = _setStreamType<ApiResponse<CoordiPostingResult>>(
      Options(
        method: 'POST',
